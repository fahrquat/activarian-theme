...
class reports_Core {
	...
	public static function fetch_incidents($paginate = FALSE, $items_per_page = 0)
	{
		...
		$exclude_params = array('c', 'v', 'm', 'mode', 'sw', 'ne', 'start_loc', 'rec');
		...
		if (isset($url_data['rec']) AND is_array($url_data['rec']))
		{
			// Sanitize each of the category ids
			$recquired_cat_ids = array();
			foreach ($url_data['rec'] as $c_id)
			{
				if (intval($c_id) > 0)
				{
					$recquired_cat_ids[] = intval($c_id);
				}
			}
				
			// Check if there are any category ids
			if (count($recquired_cat_ids) > 0)
			{
				$category_ids = implode(",", $recquired_cat_ids);
				
				array_push(self::$params, 
					' i.id IN (SELECT DISTINCT incident_id FROM '
						.$table_prefix.'incident_category WHERE category_id IN ('.$category_ids.')) '
				);
			}
		}
