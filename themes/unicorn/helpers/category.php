...
class category_Core {
	 ...
	 * @return string
	 */
	public static function get_category_tree_view($skip_cat_array = array())
	{
		...
		$sql = "SELECT c_parent.id,  COUNT(DISTINCT ic.incident_id)  AS report_count "
			...
			. "AND c_parent.category_trusted = 0 "
		...
		foreach($skip_cat_array as $skip)
		{
			unset($category_data[$skip]);
		}
