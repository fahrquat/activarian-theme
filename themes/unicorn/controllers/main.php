...
class Main_Controller extends Template_Controller {
	...
	public function __construct()
	{
		...
		if ( isset(Auth::instance()->get_user()->id) )
		{
			...
			{
				$this->template->header->header_nav->loggedin_role = 'members';
			}
			elseif(Auth::instance()->logged_in('activarian'))
			{
				$this->template->header->header_nav->loggedin_role = 'members';
			}
			elseif(Auth::instance()->logged_in('guardian'))
			{
				$this->template->header->header_nav->loggedin_role = 'members';
			}
			elseif(Auth::instance()->logged_in('admin'))
			{
				$this->template->header->header_nav->loggedin_role = 'admin';
			}
			elseif(Auth::instance()->logged_in('superadmin'))
			{
				$this->template->header->header_nav->loggedin_role = 'admin';
			}
			else 
