...
class usermsg {
	...
	public function add()
	{
		...
		if(Router::$controller == "reports" OR Router::$controller == "events" OR Router::$controller == "resources")
		{
		...
		if(Router::$controller == "profile" AND Router::$method == "user")
		{
			//create a "send report author a message" block to a report's page.
			Event::add('ushahidi_action.user_profile', array($this, '_add_msg_user'));
		}
	...
	//creates the UI for sending messages to users who added an incident.
	public function _add_msg_user()
	{
		$user = Event::$data;
		
		$username =  $user->username;
	
		$view = new View('usermsg/user_send');
