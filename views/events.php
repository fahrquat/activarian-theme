...
class Events_Controller extends Main_Controller {
	...
	public $role; //highest role the user has
	...
	public function __construct()
	{
		...
		if($this->logged_in)
		{
			...
			{
				if($role->access_level > $current_access_level)
				{
					$this->role = $role;
					$current_access_level = $this->role->access_level;
				}
			}
		}
		else
	...
	public function index()
	{
		...
		$this->themes->js = new View('events_js');
		...
		$_GET['rec']=extrauserinfohelper::$activarian_role_id;
		...
		// Category tree view
		$this->template->content->category_tree_view = category::get_category_tree_view(array(extrauserinfohelper::$roles_parent_id));
	...
	private function _get_report_listing_view($locale = '')
	{
		...
		$report_listing = new View('events_listing');
	 ...
	 */
	public function mark($id = FALSE, $saved = FALSE)
	{
		$db = new Database();
		...
		$this->template->header->this_page = 'events';
		$this->template->content = new View('events_mark');
		...
		if($this->user == null  OR $this->role == null OR ($this->role->name != 'Activarian' AND $this->role->name != 'admin'))
		{
			$this->template->content = new View('events_login');
			return;
		}
		//grab user specifics
	...
	{
		$this->template->header->this_page = 'events';
		$this->template->content = new View('events_view');
	...
	public function thanks()
	{
		...
		$this->template->content = new View('events_mark_thanks');
